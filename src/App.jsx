...
/* ─── Scroll progress across full section height ─── */
function useScrollProgress(ref) {
  ...
  useEffect(() => {
    const h = () => {
      ...
      const raw = 1 - (r.top + r.height) / (vh + r.height) + 0.15;
...
function S({ id, h = "200vh", children }) {
    ...
    <div style={{ position:"absolute", inset:0, overflow:"hidden", pointerEvents:"none" }}>
      {rings.map((r, i) => (
        <div key={i} style={{
          ...
          border:`1.5px solid ${r.color}`, opacity:0.15,
...
function Dots({ n = 25 }) {
  ...
  return <div style={{ position:"absolute", inset:0, overflow:"hidden", pointerEvents:"none" }}>
    {d.map((o,i)=><div key={i} style={{
      ...
      borderRadius:"50%", background:o.c, opacity:0.25,
        ...
        {(p) => {
          ...
          return (
            <div style={{position:"sticky",top:0,height:"100vh",display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center",textAlign:"center",overflow:"hidden"}}>
              ...
              <Blob x="-5%" y="15%" size="450px" color="rgba(0,100,255,0.06)"/>
              <Blob x="65%" y="55%" size="350px" color="rgba(123,97,255,0.05)"/>
              <Blob x="35%" y="-5%" size="300px" color="rgba(0,212,255,0.04)"/>
        ...
        {(p) => {
          ...
          return (
            <div style={{position:"sticky",top:0,height:"100vh",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",padding:"0 clamp(20px,4vw,60px)"}}>
              ...
              <Blob x="15%" y="25%" size="500px" color="rgba(0,212,255,0.05)"/>
              <Blob x="70%" y="55%" size="350px" color="rgba(123,97,255,0.04)"/>
              <div style={{position:"absolute",width:600,height:600,borderRadius:"50%",border:"1px solid rgba(0,212,255,0.08)",top:"50%",left:"35%",transform:`translate(-50%,-50%) scale(${circleScale})`,pointerEvents:"none",opacity:lerp(0.3,0,Math.max(0,p-0.7))}}/>
              <svg style={{position:"absolute",inset:0,width:"100%",height:"100%",pointerEvents:"none",opacity:0.06}}>
                ...
                <div className="grid2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:40,alignItems:"start"}}>
                  <div>
                    ...
                    <div style={{marginTop:28,opacity:bridge,transform:`translateY(${lerp(50,0,bridge)}px) scale(${lerp(0.85,1,bridge)})`,transformOrigin:"top left"}}>
                      <div style={{background:"linear-gradient(135deg,rgba(0,212,255,0.07),rgba(123,97,255,0.07))",borderRadius:20,padding:"22px 24px",border:"1px solid rgba(255,255,255,0.07)"}}>
                  ...
                  <div style={{opacity:card,transform:`scale(${lerp(0.5,1,card)}) translateY(${lerp(80,0,card)}px) rotateY(${lerp(-12,0,card)}deg)`,transformOrigin:"center",perspective:800}}>
                    <div style={{background:"rgba(255,255,255,0.035)",backdropFilter:"blur(20px)",borderRadius:24,padding:"34px 28px",border:"1px solid rgba(255,255,255,0.08)"}}>
                      ...
                      <div style={{marginTop:20,display:"flex",flexWrap:"wrap",gap:7}}>
                        {["Urban Sprawl","Street Life","Public Transit","Social Fabric"].map((t,i)=>(
                          <span key={t} style={{padding:"4px 12px",borderRadius:50,fontSize:10,color:"#00d4ff",background:"rgba(0,212,255,0.07)",border:"1px solid rgba(0,212,255,0.15)",opacity:tagP(i),transform:`translateY(${lerp(20,0,tagP(i))}px) scale(${lerp(0.7,1,tagP(i))})`}}>{t}</span>
                        ))}
                      </div>
                    </div>
                  </div>
        ...
        {(p) => {
          ...
          return (
            <div style={{position:"sticky",top:0,height:"100vh",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",padding:"0 clamp(20px,4vw,60px)"}}>
              ...
              <Blob x="55%" y="15%" size="400px" color="rgba(123,97,255,0.05)"/>
              <Blob x="10%" y="65%" size="350px" color="rgba(0,212,255,0.04)"/>
              <FloatingRings count={3}/>
              <Dots n={20}/>

              <svg style={{position:"absolute",right:"6%",top:"12%",width:200,height:200,opacity:0.07,pointerEvents:"none"}}>
              ...
              <div style={{maxWidth:1000,width:"100%",zIndex:2,opacity:Math.max(0,1-exit)}}>
                ...
                <div className="grid2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:40,alignItems:"start"}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:14}}>
                    {[
                      ...
                      return (
                        <div key={i} style={{
                          background:"rgba(255,255,255,0.035)",borderRadius:18,padding:"18px 22px",
                          border:"1px solid rgba(255,255,255,0.07)",opacity:cp,
                ...
                <div style={{marginTop:32,textAlign:"center",opacity:quote,transform:`scale(${lerp(0.8,1,quote)}) translateY(${lerp(40,0,quote)}px)`}}>
                  <div style={{background:"rgba(255,255,255,0.03)",borderRadius:24,padding:"28px 32px",border:"1px solid rgba(255,255,255,0.06)"}}>
        ...
        {(p) => {
          ...
          return (
            <div style={{position:"sticky",top:0,height:"100vh",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",padding:"0 clamp(20px,4vw,60px)"}}>
              <GridBg opacity={0.035}/>
              <Dots n={15}/>
              <Blob x="5%" y="35%" size="400px" color="rgba(0,212,255,0.04)"/>
              <Blob x="75%" y="15%" size="300px" color="rgba(123,97,255,0.05)"/>
              <FloatingRings count={4}/>
              ...
              <div style={{maxWidth:1000,width:"100%",zIndex:2,opacity:Math.max(0,1-exit)}}>
                ...
                <div className="grid2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:40,alignItems:"center"}}>
                  <div style={{opacity:txt,transform:`translateY(${lerp(50,0,txt)}px)`}}>
                    <p style={{fontSize:15,lineHeight:1.8,color:"rgba(245,245,247,0.6)",marginBottom:18}}>
                          ...
                          {bh>15 && Array.from({length:Math.floor(bh/18)}).map((_,j)=>
                            Array.from({length:Math.floor(b.w/12)}).map((_,k)=>
                              <rect key={`${j}${k}`} x={b.x+4+k*12} y={200-bh+6+j*18} width={4} height={8} rx={1}
                                fill={Math.random()>.4?"rgba(0,212,255,0.2)":"rgba(250,204,21,0.15)"}/>
                            )
                          )}
                        </g>;
                      })}
                ...
                <div className="grid2" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,marginTop:28}}>
                  {[
                    ...
                    return <div key={i} style={{
                      background:"rgba(255,255,255,0.035)",borderRadius:18,padding:"20px 18px",
                      border:"1px solid rgba(255,255,255,0.07)",opacity:cp,
                      transform:`translateY(${lerp(60,0,cp)}px) scale(${lerp(0.7,1,cp)})`,
                    }}>
                      <div style={{fontSize:24,marginBottom:8,transform:`scale(${lerp(0.2,1,cp)})`,display:"inline-block"}}>{c.icon}</div>
                      <div style={{fontSize:13,fontWeight:600,marginBottom:4}}>{c.title}</div>
                      <div style={{fontSize:11,color:"rgba(245,245,247,0.45)",lineHeight:1.6}}>{c.text}</div>
                    </div>;
                  })}
                </div>
              </div>
        ...
        {(p) => {
          ...
          return (
            <div style={{position:"sticky",top:0,height:"100vh",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",padding:"0 clamp(20px,4vw,60px)"}}>
              ...
              <Dots n={25}/>
              <div style={{position:"absolute",width:500,height:500,borderRadius:"50%",background:"radial-gradient(circle, rgba(123,97,255,0.07) 0%, transparent 65%)",top:"50%",right:"5%",transform:`translate(0,-50%) scale(${bgExpand})`,pointerEvents:"none"}}/>
              <Blob x="10%" y="55%" size="350px" color="rgba(0,102,255,0.04)"/>
              ...
              <div style={{maxWidth:1000,width:"100%",zIndex:2}}>
                ...
                <div className="grid2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:40,alignItems:"start"}}>
                  ...
                  <div style={{opacity:list,transform:`scale(${lerp(0.75,1,list)}) translateY(${lerp(50,0,list)}px)`,background:"linear-gradient(135deg,rgba(0,102,255,0.06),rgba(0,212,255,0.06))",borderRadius:22,padding:"28px 26px",border:"1px solid rgba(0,212,255,0.12)"}}>
